...
2. 配置hook:
    - logrus可以注册多个hook
    - 在logrus里，想写一个hook，就必须同时实现 `Fire()` 和 `Levels()`
        - `Levels()`: 哪些日志级别需要执行这个 Hook。
        - `Fire()`: 决定触发后做什么
    - 需要实现产生日志之后，把它写入到日志文件中去
    - 按天分片
    - 错误的日志单独存放
    - 具体看`core/logrus.go` - `rotateFiles` - `Fire`

- 将之前的fmt.Println更改为logrus.Infof/Errorf/Fatalf
    - 打印error日志 ![alt text](image-3.png)
